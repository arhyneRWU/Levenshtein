...
#include <mysql/mysql.h>  // MySQL UDF headers; ensure this path is correct
#include <iomanip>        // For std::setw, std::left, std::fixed, std::setprecision

#include "benchtime.hpp"  // Timer class for benchmarking
...
// damlevconstmin functions
int damlevconstmin_init(UDF_INIT *initid, UDF_ARGS *args, char *message);
long long damlevconstmin(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error);
void damlevconstmin_deinit(UDF_INIT *initid);

// damlevconst functions
...
std::vector<UDF_Function> get_udf_functions() {
    ...
    // From CMake:
    // AVAILABLE_ALGORITHMS "damlev;damlevconstmin;damlevconst;damlevlim;damlevp;noop"
    // ALGORITHM_ARGS "2;3;3;3;3;1"

    std::vector<std::string> algorithms = { "damlev", "damlevconstmin", "damlevconst", "damlevlim", "damlevp", "noop" };
    ...
    for(size_t i = 0; i < algorithms.size(); ++i) {
        ...
        else if(udf.name == "damlevconstmin") {
            udf.init = &damlevconstmin_init;
            udf.func = &damlevconstmin;
            udf.deinit = &damlevconstmin_deinit;
        }
        else if(udf.name == "damlevconst") {
            udf.init = &damlevconst_init;
            udf.func = &damlevconst;
            udf.deinit = &damlevconst_deinit;
        }
...
int main(int argc, char *argv[]) {
    ...
    unsigned maximum_size = WORD_COUNT;  // Number of word pairs to benchmark
    boost::interprocess::file_mapping text_file;
    std::string openedFilePath;

    // Attempt to open the primary file; fallback if necessary
    try {
